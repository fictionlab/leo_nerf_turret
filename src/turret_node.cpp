...
void joyCallback(const sensor_msgs::Joy::ConstPtr & msg){
  ...
  if(!msg->buttons[7]){return; }                                //turret enable button

  //Shooting
  bool_msg.data = msg->buttons[8];                              //Left trigger - spin the motors to build inertia if pressed
  spin_pub.publish(bool_msg);

  if( bool_msg.data && msg->buttons[9]){                        //if both Left and RIGHT trigger are pressed - use the other motor to fire the balls
    bool_msg.data = true;
  ...
  float msg_speed_x = msg->axes[2];                               //Get analog position for pan control
  float msg_speed_y = msg->axes[3];                               //Get analog position for tilt control
  ...
  //Tilt
  updateDynamixel(1,speed_id1,1450,2048);                          //Hardcoded min and max position for tilt as the last two parametes :) 
...
int main(int argc, char ** argv)
{
  ...
  changeVelocityPIValues(1, 400, 3000);
  changeVelocityPIValues(2, 400, 3000);

  changePositionPIDValues(1, 200, 0, 3600);
  changePositionPIDValues(2, 200, 0, 3600);

  readVelocityPIValues(1);
  readVelocityPIValues(2);
  readPositionPIDValues(1);
  readPositionPIDValues(2);
